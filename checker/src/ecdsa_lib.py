...
def inverse_mod(a_num: int, m_mod: int):
...
def sha2(raw_message: bytes):
    # SHA-2 256
    return hashlib.sha256(raw_message).digest()


def bytes_to_int(bytes_data: bytes):
    return int.from_bytes(bytes_data, "big")


def sha2_int(data: bytes):
    return bytes_to_int(sha2(data))


def curve_size(curve_name: str):
...
def check_publickey(pubkey: tuple[int, int], curve_str: str):
...
def privkey_to_pubkey(pv_key_int: int, curve_name: str):
...
def ecdsa_sign_kout(z_hash: int, pvkey: int, curve_name: str):
...
def scalar_mult_x(d_scalar: int, curve: str):
